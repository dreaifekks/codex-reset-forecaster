...
import { sourceRoleForIdentity } from "../core/sources.mjs";
...
const TARGET_RESET_TERMS = /\b(codex|global|all paid|paid users|paid plans|chatgpt work)\b/i;
const GLOBAL_TERMS = /\b(all paid|paid users|paid plans|all plans|all accounts|all users|all codex users|all paid chatgpt subscriptions|everyone(?:'s)?|everybody|across all|global|codex and chatgpt work|chatgpt work and codex|all (?:our )?(?:chatgpt work and codex|codex and chatgpt work) users)\b/i;
...
export function extractSignal(observation, config, {
  ...
  const global = GLOBAL_TERMS.test(text);
  ...
  const explicit = phase !== "rumor" && (global || !["quota_reset", "quota_refill"].includes(eventType));
  ...
  return createRecord({
    ...
    data: {
      ...
      claim: {
        ...
        scope: targetScope({
          ...productScope,
          population: global ? "platform" : "unknown",
          plans: global ? ["paid"] : ["unknown"],
          quota_bucket: global ? config.target.quota_bucket : null,
        }),
