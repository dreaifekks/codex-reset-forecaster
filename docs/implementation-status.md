...
| Codex experience taxonomy | Relevant issues and recoveries carry category, severity, lifecycle, affected scope/surfaces, workaround, and evidence basis; they remain visible for audit while initially excluded from the forecast vector | relevance, extraction, API, and website tests |
...
| Forecast website | 4h, 24h, and 7×24 projections of the same prediction, green intensity, exact tooltips, provenance/source freshness, separate Codex-experience and competition evidence groups, and a deprecated one-version `community` API alias | API integration test and Playwright browser audit |
