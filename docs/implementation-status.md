...
| Publication event ledger | Non-canonical append-only `publication-event/1` audit rows with exact source revisions, stable event/entity IDs, contiguous sequence cursor, baseline-only initialization, expiry, outcome-first ordering, hysteretic experimental watches, correction links, and replay idempotency | publication schema/example validator and notification-projector tests |
