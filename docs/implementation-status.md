...
| Exact authority timing | One latest exact configured-author statement mixes versioned prior first-event mass into its asserted interval without also entering the learned intent/overlap vector; its phase prior controls total interval mass while a 0.5-power tempered complete-range baseline controls intraday shape, including fractional boundary slots and a zero-baseline duration fallback; an allowlisted direct future reply may use its exact parent for target/platform scope only while the child supplies scheduled phase and time; summaries and inherited quote evidence are excluded, denials cancel old windows, expired windows deactivate, compatible completed outcomes consume the intent and re-anchor recurrence, and four-hour walk-forward scoring uses the same complete-range allocation basis as live forecasts | extraction and authority-timing tests |
...
| Forecast website | 4h, 24h, and 7×24 projections of the same prediction, green intensity, exact tooltips, distinct current-source health, historical-label maturity delay, and sample sufficiency rather than one overall-health score, an exact-source Tibo timeline (including unclassified posts), ranked impact episodes explicitly separated from reset probability, separate Codex-experience and competition evidence groups, a deprecated one-version `community` API alias, and a notification read-model dialog backed by 28-horizon last-good/SWR profiles, an immediate final slider load, a four-standard-deviation display range, and a two-standard-deviation new-browser suggestion; all public display data comes from an atomically replaced in-memory last-good read model and never falls back to canonical scans | API integration, pipeline-worker, profile-worker, and UI-helper tests; browser audit remains a release check |
| Historical-results website | Latest eligible confirmed outcomes with occurrence and verification times plus exact official or operator provenance; operator rows are visibly `silver`, and the page remains independent of evaluation availability and exposes no model-performance claims | API integration, manual-reset, and UI-helper tests; Playwright browser audit remains a release check |
| Publication event ledger | Non-canonical append-only `publication-event/1` audit rows with exact source revisions, stable event/entity IDs, contiguous sequence cursor, baseline-only initialization, occurrence-bounded first-confirmation expiry, material public corrections only, outcome-first ordering, hysteretic experimental watches, correction links, and replay idempotency | publication schema/example validator and notification-projector tests |
| Atom and Web Push | Default Atom feed and push topics contain authority/outcome events only; the shared experimental feed remains explicit, while parameterized Atom and opt-in Web Push apply one versioned 1..168h horizon/threshold rule over a bounded eligible-forecast input stream with silent baseline, hysteresis, expiry, and per-subscription cursor; the UI exposes the same rule plus an as-issued historical distribution/reliability profile; Web Push is disabled until a protected persistent VAPID key file is mounted and public subscription mutations have edge rate-limit/challenge protection | notification-feed, subscription-policy, probability-profile, web-notifications, UI-helper, and web-push tests |
| Telegram bot | One public Bot accepts ordinary users only in their own private chats, keeps explicit groups read-only, separates administrator commands and operations alerts, rate-limits ordinary commands, supports stable plus versioned 1..168h/1%..99% probability subscriptions (`experimental` remains a 4h/50% alias), silently baselines/reset-recovers its forecast-input cursor, rejects events emitted before each dynamic subscription started, requests sparse points only for distinct active horizons (tail/gate only when none), binds queued probability jobs to preference generation/hash and current outcome gate, defaults presentation to Asia/Tokyo/UTC+9 while canonical time stays UTC, and keeps bot-ID-bound single-writer delivery state | telegram-runtime tests and container health check |
| Origin traffic operations | Privacy-preserving one-minute/UTC-day route aggregates, complete-day growth against a seven-day median, five-minute user-impact/resource-pressure evaluation, lightweight liveness, protected admin summary, and an independent transition-alert cursor that never enters model/publication data | traffic-monitor and Telegram operations tests |
| Ten-minute operation | Non-overlapping 10-minute collection and hourly-slot feature/forecast refresh in a dedicated worker isolate, exact coverage-recheck wakeups, at-most-daily batch train/evaluate attempt, frozen training cutoff, stable model fallback, atomic `serving-snapshot/3` publication, and uninterrupted HTTP service from the prior read model while work runs or fails | scheduler, pipeline-worker, read-model, pipeline, and Docker smoke tests |
